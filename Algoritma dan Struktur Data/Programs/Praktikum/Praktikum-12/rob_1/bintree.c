...
int rob1(BinTree root){
    int ruang;
    scanf("%d",&ruang);
    return rampok_path(root);
}

/* ROB 2 */
int rampok_path_2(BinTree t){
    if (t == NULL) {
        return 0;
    }
    int left  = rampok_path(LEFT(LEFT(t))) + rampok_path(RIGHT(LEFT(t)));
    int right = rampok_path(LEFT(RIGHT(t))) + rampok_path(RIGHT(LEFT(t)));
    return (left > right ? left : right) + ROOT(t); 
}
